...
void ClapTrap::beRepaired(unsigned int amount) {
  if (amount > energy_points_) {
    std::cout << name_ << "'s energy recovery: FAILED! [" << amount
              << " is greater than energy points.]" << std::endl;
    return;
  }
  std::cout << name_ << " recovered its energy by " << amount << "!"
            << std::endl;
  std::cout << "[" << hit_points_ << "] --> "
            << "[" << hit_points_ + amount << "]" << std::endl;
...
std::string const &ClapTrap::getName() const { return name_; };
...
void ClapTrap::setEnergyPoints(unsigned int energy_points) {
  ...
  std::cout << "-> Energy Points updated [" << energy_points_ << "]"
            << std::endl;
};
void ClapTrap::setAttackDamage(unsigned int attack_damage) {
  attack_damage_ = attack_damage;
  std::cout << "-> Attack Damage updated [" << attack_damage_ << "]"
            << std::endl;
};